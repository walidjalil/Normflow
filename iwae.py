...
class IWAE(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=32, kernel_size=3, n_latent=None, n_samples=5):
    ...
    def forward(self, x):
        ...
        mu = self.mu(flat)
        mu = mu.repeat(self.n_samples, 1, 1).permute(1, 0, 2)
        log_var = self.var(flat)
        log_var = log_var.repeat(self.n_samples, 1, 1).permute(1, 0, 2)

        z = self.reparam_trick(mu, log_var)

        b = z.shape[0]
        z = z.view(-1, self.n_latent)
        ...
        output = output.view(b, -1, output.shape[1], output.shape[2], output.shape[3])
